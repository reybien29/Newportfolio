    ...
    public function test_welcome_page_returns_portfolio_props_with_ready_github_activity(): void
    {
        ...
        $response
            ...
            ->assertInertia(fn (AssertableInertia $page) => $page
                ...
                ->has('portfolio.stack.items', 16)
                ->where('portfolio.stack.items.0.name', 'Laravel')
                ->where('portfolio.stack.items.0.category', 'Framework')
                ->where('portfolio.stack.items.1.name', 'Codex')
                ->where('portfolio.stack.items.2.name', 'Claude Code')
                ->where('portfolio.stack.items.3.name', 'TypeScript')
                ->where('portfolio.stack.items.4.name', 'Figma')
                ->where('portfolio.stack.items.5.name', 'Cursor')
                ->where('portfolio.stack.items.8.name', 'Notion')
                ->where('portfolio.stack.items.12.name', 'Vue')
                ...
                ->where('portfolio.projects.0.name', 'Contact Management System')
                ->where('portfolio.projects.0.liveUrl', 'https://konek-main-qdmf3m.free.laravel.cloud/')
                ->where('portfolio.projects.1.name', 'Tournament Management System (TMS)')
                ->where('portfolio.projects.1.liveUrl', 'https://event-tkku.onrender.com/')
                ->where('portfolio.projects.2.name', 'Jusa’s Tropical Smoothie Café')
                ->where('portfolio.projects.2.liveUrl', 'https://jusas.onrender.com/')
